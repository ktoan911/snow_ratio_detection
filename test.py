...
def preprocess_like_train(
    img: Image.Image, mask: Image.Image, do_augment: bool, seed: int = 42
):
    """
    ...
    if do_augment:
        ...
        img = TF.resized_crop(
            img, i, j, h, w, (IMG_SIZE, IMG_SIZE), TF.InterpolationMode.BILINEAR
        )
        mask = TF.resized_crop(
            mask, i, j, h, w, (IMG_SIZE, IMG_SIZE), TF.InterpolationMode.NEAREST
        )
    ...
    mask_bin = torch.from_numpy((mask_arr > 0).astype(np.float32)).unsqueeze(0)
...
def save_debug_images(
    ...
    Image.fromarray(mask_arr.astype(np.uint8), mode="L").save(
        out_dir / f"{stem}_mask_raw.png"
    )
...
def main():
    ...
    ap.add_argument(
        ...
        default="train",
        choices=["train", "val"],
        help="Test preprocess train (augment) hoặc val (no augment)",
    )
    ap.add_argument(
        "--limit", type=int, default=0, help="Giới hạn số ảnh để test (0=all)"
    )
    ...
    ap.add_argument(
        "--topk", type=int, default=20, help="In top K ảnh có fg_ratio thấp nhất"
    )
    args = ap.parse_args()
    ...
    if not pairs:
        raise SystemExit(
            "Không tìm thấy cặp ảnh–mask nào (cần <stem>.png trong label_dir)."
        )
    ...
    for idx, (img_path, mask_path) in enumerate(pairs):
        ...
        _, mask_bin, mask_arr = preprocess_like_train(
            img, mask, do_augment=do_augment, seed=args.seed + idx
        )
        ...
        if args.save_debug:
            ...
            if fg_pixels == 0 and zero_count <= 5:
                save_debug_images(
                    ...
                    mask_arr,
                    mask_bin,
                    stem=f"ZERO_{zero_count:02d}_{img_path.stem}",
                )
            if fg_pixels > 0 and idx < 3:
                save_debug_images(
                    ...
                    mask_arr,
                    mask_bin,
                    stem=f"NORMAL_{idx:02d}_{img_path.stem}",
                )
    ...
    print(
        f"FG ratio min/mean/max: {np.min(fg_ratios):.6f} / {np.mean(fg_ratios):.6f} / {np.max(fg_ratios):.6f}"
    )
